...
import PendingRequestsActions from "./PendingRequestsActions";


export default function PendingRequests({ data, columns }) {
  //Pagination
  ...
  useEffect(() => {
    const handleFilter = () => {
      const selected = data?.filter((where) => where.status === "pending");
  ...
  const downloadAsPDF = () => {
    const doc = new jsPDF();
    doc.text("SHARP APP PENDING REQUESTS LIST", 20, 10);
    doc.autoTable({
      head: [
        [
          "S/N",
          ...
          <TableBody>
            {slicedData.reverse().map((row, index) => (
              <TableRow key={index}>
                ...
                <PendingRequestsActions data={data} row={row} />
              </TableRow>
            ))}
          </TableBody>
        </Table>
